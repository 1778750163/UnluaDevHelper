...
TSharedRef< ISlateStyle > FTestStyle::Create()
{
	...
	// RichText Test Showcase
	{
		...
		Style->Set( "RichText.RoundedBackground", new BOX_BRUSH( "Common/RoundedSelection_16x", 4.0f/16.0f, FLinearColor(FColor(0x0c0c0cff)) ) );
		...
		//Interactive example
		{
			Style->Set( "RichText.Interactive.Details.Background", new BOX_BRUSH( "Testing/FlatColorSquare", FVector2D(1.0f, 1.0f), FMargin(0), FLinearColor(FColor(0xff282c34))) );
		...
		// Editor example
		{
			...
			// Toolbar
			{
				...
				const FLinearColor TextColor(FColor(0xffabb2bf));
